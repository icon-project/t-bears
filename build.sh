...
if [[ ("$1" = "test" && "$2" != "--ignore-test") || ("$1" = "build") || ("$1" = "deploy") ]]; then
  ...
  rm -rf earlgrey*
  wget "http://tbears.icon.foundation.s3-website.ap-northeast-2.amazonaws.com/iconCommon-0.9.4-py3-none-any.whl" -P $VER
  pip install --force-reinstall $VER/iconCommon-0.9.4-py3-none-any.whl
  rm -rf iconCommon*
